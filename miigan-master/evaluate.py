...
ssim_obj = SSIM(channel=1,size_average=True,data_range=2.)
lpips_obj = LPIPS(net='alex')
L1_obj = torch.nn.L1Loss()
L2_obj = torch.nn.MSELoss()
metrics = {}
visualize = False
class_agn = True
...
for i, data in tqdm(enumerate(dataloader), total=int(len(dataloader))):
    ...
    # mse = torch.mean((model.real_B - model.fake_B) ** 2)
    # current_psnr = 20 * torch.log10(torch.tensor(2.0)) - 10 * torch.log10(mse)
    # psnr = (psnr * i + current_psnr.item()) / (i + 1)

    l2  = L2_obj(model.real_B, model.fake_B).item()
    ...
    if class_agn:
        for k in dataset_class.keys():
            if k in cls:
                ...
                metrics[dataset_class[k]]['l2'].update(l2, batch_size)
    ...
    overall_metric['l2'].update(l2, batch_size)
