...
class VMEncoder(nn.Module):
    ...
    def forward_features(self, x, xab=None):
        # print(f"Input shape: {x.shape}")
        skip_list = []

        if xab is not None:
            # 假设 xab 是一个列表，包含了 x0, x1, x2, x3 等特征
            x0, x1, x2, x3 = xab
            # print(f"Using provided xab features: x0 shape: {x0.shape}, x1 shape: {x1.shape}, x2 shape: {x2.shape}, x3 shape: {x3.shape}")
        ...
        for i, layer in enumerate(self.layers):
            ...
            # print(f"   After VSSLayer {i}: {x.shape}")
            if xab is not None:
                # 确保形状一致，可以使用插值或者其它方法进行维度匹配
                if x.shape == x0.shape:  # 假设此时 x 经过了第一层
                    x = x + x0
                elif x.shape == x1.shape:  # 对应第二层
                    x = x + x1
                elif x.shape == x2.shape:  # 对应第三层
                    x = x + x2
                elif x.shape == x3.shape:  # 对应第四层
                    x = x + x3

        return x, skip_list

    def forward(self, x, xab=None):
        if x.size()[1] == 1:
            x = x.repeat(1, 3, 1, 1)

        x, skip_list = self.forward_features(x, xab)
        return x, skip_list

# class VisionMamba(nn.Module):
#     def __init__(self, config):
#         super().__init__()
#         self.mamba = VMEncoder(config)
#
#     def forward(self, x):
#         if x.size()[1] == 1:
#             x = x.repeat(1, 3, 1, 1)
#         x, skip_list = self.mamba(x)
#         return x, skip_list
#
#
# CONFIGS = {
...
#
# model = VisionMamba(cf).cuda()
# total = sum([param.nelement() for param in model.parameters()])
# # 精确地计算：1MB=1024KB=1048576字节
# print('VisionMamba Number of parameter: % .4fM' % (total / 1e6))
# encoded, feature_list = model(x.cuda())
# print("VisionMamba Output shape:", encoded.shape)
# for f in feature_list:
#     print("VisionMamba shape:", f.shape)
